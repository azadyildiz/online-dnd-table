...
const createCharacter = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(401)
        .json({ message: 'Invalid token. Cannot found this user.' })
    }
    ...
    if (!campaign.players.includes(userId)) {
      return res
        .status(401)
        .json({ message: 'Player is not connected to this Campaign.' })
    }
...
const updateCharacter = async (req, res) => {
  try {
    ...
    if (!campaign) {
      return res
        .status(401)
        .json({ message: 'Invalid campaign. Cannot found this campaign.' })
    }
    ...
    if (campaign.dm.toString() !== userId) {
      return res
        .status(401)
        .json({ message: 'You cannot update this character. You are not DM.' })
    }
...
const getCharacter = async (req, res) => {
  try {
    ...
    if (!campaign) {
      return res
        .status(401)
        .json({ message: 'Invalid request. Cannot found this campaign.' })
    }
    ...
    if (!character) {
      return res
        .status(401)
        .json({ message: 'Invalid request. Cannot found this character.' })
    }

    // Check if user is campaign's player or dm
    if (
      !(
        campaign.players.includes(user._id) ||
        campaign.dm.toString() === user._id.toString()
      )
    ) {
      return res
        .status(401)
        .json({ message: 'This user is not in this campaign.' })
    }
    ...
    if (!campaign.characters.includes(character._id)) {
      return res
        .status(401)
        .json({ message: 'This character is not in this campaign.' })
    }
...
const getAllCharacters = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({
        message: 'All characters in this campaign have been pushed.',
        characters
      })
  } catch (error) {
    res.status(500).json({ message: error.message })
