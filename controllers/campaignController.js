...
const joinCampaign = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: 'User joined the campaign.', campaign: campaign })
  } catch (error) {
    res.status(500).json({ message: error.message })
  }
}
...
const deleteCampaign = async (req, res) => {
  try {
    ...
    if (campaign.dm.toString() !== userId) {
      return res
        .status(401)
        .json({ message: 'You cannot delete this campaign. You are not DM.' })
    }
...
const updateCampaign = async (req, res) => {
  try {
    ...
    if (campaign.dm.toString() !== userId) {
      return res
        .status(401)
        .json({ message: 'You cannot updated this campaign. You are not DM.' })
    }
    ...
    return res
      .status(200)
      .json({ message: 'Campaign updated.', campaign: campaign })
  } catch (error) {
    res.status(500).json({ message: error.message })
  }
}
