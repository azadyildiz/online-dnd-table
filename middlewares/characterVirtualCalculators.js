...
async function hitPointsCalculator(model) {
    try {
        var hitPoints = diceRoller.maxRoller(model.hitPointsDice) + model.abilityModifiers.constitution;
        ...
        if(model.level > 1){
            if(playerHitDiceRollType === 'min'){
                hitPoints = hitPoints + ((model.level - 1) * (diceRoller.minRoller(model.hitPointsDice) + model.abilityModifiers.constitution));
                return hitPoints;
            }

            if(playerHitDiceRollType === 'average'){
                hitPoints = hitPoints + ((model.level - 1) * (diceRoller.averageRoller(model.hitPointsDice) + model.abilityModifiers.constitution));
                return hitPoints;
            }
            if(playerHitDiceRollType === 'max'){
                hitPoints = hitPoints + ((model.level - 1) * (diceRoller.maxRoller(model.hitPointsDice) + model.abilityModifiers.constitution));
                return hitPoints;
            }
            if(playerHitDiceRollType === 'random'){
                hitPoints = hitPoints + ((model.level - 1) * (diceRoller.randomRoller(model.hitPointsDice) + model.abilityModifiers.constitution));
                return hitPoints;
            }
        }
        else{
